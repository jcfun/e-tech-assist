  ...
  <a-modal v-model:visible="visible" :closable="false" :align-center="false">
    <a-tabs default-active-key="1" size="small">
      ...
      <a-tab-pane key="2" title="站外">
        <div class="p-4">
          <a-input-search v-model="outValue" placeholder="请输入关键字" press-enter allowClear size="large" @search="onOutSearch" />
        </div>
      </a-tab-pane>
    </a-tabs>
  </a-modal>
...
<script lang="ts">
  ...
  export default defineComponent({
    setup() {
      ...
      onMounted(() => {
        searchList.value = transformRoutes(permStore.getPermSideBar);
      });
      return {
        visible,
        activeKey,
