<template>
  <a-drawer v-model:visible="opened" placement="right" title="系统设置" closable :width="appStore.deviceType === 'mobile' ? '75%' : '280px'">
    <Scrollbar class="wrapper">
      ...
      <a-row :gutter="[10, 10]">
        <a-col v-for="(item, index) of primartyColorList" :key="index" :span="4">
          <div class="color-wrapper" :class="{ circle: item.checked }" :style="{ backgroundColor: item.value }" @click="colorClick(item)"></div>
        </a-col>
      </a-row>
      <div style="height: 20px"></div>
...
<script lang="ts">
  ...
  import type { ModalDialogType } from '@/components/types';
  ...
  import { ThemeMode } from '@/store/types/layout';
  export default defineComponent({
    ...
    setup() {
      ...
      function onShowTabbar(_val: boolean) {
