<template>
  <div class="vaw-inner-side-bar-wrapper" :style="{ borderRadius: '0px' }" :class="[!appStore.isCollapse ? 'open-status' : 'close-status', bgColor]">
...
<script lang="ts">
  ...
  import { SideTheme, ThemeMode } from '@/store/types/layout';
    ...
    setup() {
      ...
      watch(
        ...
        () => {
          ...
          const item = permStore.getPermSideBar.find(it => it.path === firstMatchPath);
