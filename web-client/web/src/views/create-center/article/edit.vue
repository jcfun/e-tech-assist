...
<script setup lang="ts">
  ...
  import type { UpdateArticleDTO } from '@/api/types/create-center/article';
  ...
  articleTitle.value = articleDetailStore.articleDetail.title;
  // --------------------编辑器开始------------------------
  // 初始化 MENU_CONF 属性
  ...
  // 内容 HTML
  const valueHtml = ref('');
  valueHtml.value = articleDetailStore.articleDetail.content;
  const toolbarConfig = {};
  const handleChange = () => {
  ...
  // --------------------文章封面开始------------------------
  const cover = ref('');
  cover.value = articleDetailStore.articleDetail.cover;
  // 图片裁剪
  ...
  const onFinish = async (status: string) => {
    const dto = {
      id: articleDetailStore.articleDetail.id,
      title: articleTitle.value,
      content: valueHtml.value,
      cover: cover.value,
      status,
    } as UpdateArticleDTO;
    let res = await article.updateArticle(dto);
    if (res.code == 200) {
