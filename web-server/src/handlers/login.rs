...
pub async fn captcha() -> Result<Res<Captcha>, MyError> {
    ...
    set_string_ex(
        &captcha.uuid,
        &captcha.captcha,
        APP_CFG.get().unwrap().captcha.exp,
    )
    .await?;
...
pub async fn login_wxapp(
    ...
    let res = wxapp::get_session_key(
        &APP_CFG.get().unwrap().wxapp.appid,
        &APP_CFG.get().unwrap().wxapp.secret,
        &payload.code.unwrap(),
    )
    .await?;
    // 构建登录日志所需数据
    ...
    // 根据openid查询用户信息
    let res =
        get_user_info_by_openid(&APP_CONTEXT.get().unwrap().db, &String::from(openid)).await?;
    // 判断是否已经授权过微信登录
    if res.is_none() {
...
pub async fn wxapp_register(
    ...
    if count.rows_affected != 0 {
        ...
        if count.rows_affected != 0 {
            // 通过用户openid获取用户数据
            let res =
                get_user_info_by_openid(&APP_CONTEXT.get().unwrap().db, &payload.openid.unwrap())
                    .await?;
            let mut user_info = res.unwrap();
    ...
    // 通过用户openid获取用户数据
    let res =
        get_user_info_by_openid(&APP_CONTEXT.get().unwrap().db, &payload.openid.unwrap()).await?;
    let mut user_info = res.unwrap();
    user_info.openid = Some(register_dto.openid.unwrap());
