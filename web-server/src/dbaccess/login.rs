...
/// 根据用户账号密码查询用户信息
#[py_sql(
    "select a.id, a.operate_time, a.operator, a.operator_id, a.create_time, a.creator, a.creator_id, a.delete_flag, a.account, a.disable_flag, a.detail_id, a.role_id, a.description, a.openid, b.phone_number, b.email, b.nickname, b.avatar_url, b.last_login_time, b.last_login_ip, b.gender, b.language, b.country, b.province, b.city from t_user a join t_user_detail b on a.detail_id = b.id where (a.account = #{identity} or b.email = #{identity} or b.phone_number = #{identity} ) and a.password = #{password}",
)]
...
/// 新增用户信息
#[py_sql(
    "`insert into t_user(id, operate_time, operator, operator_id, create_time, creator, creator_id, delete_flag, account, password, detail_id, openid) `
    values
    (
...
/// 新增登录日志
#[py_sql(
    "`insert into t_login_log(id, operate_time, operator, operator_id, create_time, creator, creator_id, delete_flag, identity, status, description, user_agent, ip, ip_addr, mac, method ) `
    `values`
        (
...
#[py_sql(
    ...
    `where `
        ...
        `or b.phone_number = #{dto.user_id} `
        `or b.email = #{dto.user_id}) `
        `and b.phone_number = #{dto.user_id}`"
)]
...
// 通过手机号/用户账号查询用户数量
#[py_sql(
    "select count(*) from t_user a join t_user_detail b on a.detail_id = b.id where a.account = #{identity} or b.phone_number = #{identity}"
)]
...
/// 新增微信注册用户详情信息
#[py_sql(
    "`insert into t_user_detail(id, operate_time, operator, operator_id, create_time, creator, creator_id, delete_flag, phone_number, nickname, avatar_url, gender, language, country, province, city) `
    values
        (
